// x 軸方向に傾けるスキューのプログラムをバイリニア補間を用いて実現せよ．第1 引数に入力画像，
// 第 2 引数に出力画像，第 3 引数に傾ける角度 θ を取るものとする．
// 画像の座標軸をどう取ったのか傾けるのか，角度をどちらにとったのか等のプログラムの仕様をソースコード中にコメント形式で記述すること．
...
int main(const int argc,const char *argv[]){
  ...
  //新しい画像を用意
  im_new= gdImageCreateTrueColor(width+height*tangent,height);

  ratio=(width+height*tangent)/width;

  for(i=0;i<(int)width+height*tangent;i++){
    for(j=0;j<height;j++){
      
      double_x=i-j*tangent;
      double_y=j;
      int_x=(int)double_x;
      int_y=(int)double_y;
