// 画像全体に対して，傾き 45 度の直線以外の何らかのトーンカーブによる補正を施し，結果をファイルとして出力するプログラムを作成せよ．
// ただし，RGB 全ての成分に対して同じトーンカーブを適用すること．これまでの課題と同様，第 1 引数で入力ファイル名を，第 2 引数で出力ファイル名を指定できるようにすること．
