/*
平面上の点集合に対して最小全域木 MST を求める貪欲法アルゴリズムを実装し
ます。どの二つの点の間にも辺があり、その重みは平面上の距離（ユークリッド
距離）を整数に切り捨てたものであるとします。

今回は、下敷きプログラムがありません。前回の MSTByBacktrack をベース
にしてクラス MSTGreedy を自分で作成してください。
MSTByBacktrack の Backtrack ボタンに加えて Greedy ボタンを持ち、この
ボタンを押すと貪欲法によりＭＳＴを作成するようにしてください。Backtrack
ボタンとその処理はそのまま残して、結果の比較ができるようにしてください。
*/
...
//* 2つのアルゴリズムの実行時間を比較すると、
//* 頂点の数が少ないときにはほとんど実行時間の差はみられないが、
//* 頂点の数が増えるにつれてバックトラック法での実行時間が指数的にどんどん長くなっていき、
//* 頂点の数が9個あたりから実行時間の差が見てわかるほどに顕著になる。
//* 貪欲法での実行時間は頂点の数が1000個になっても大きな変化は見られなかった。
public class MSTGreedy extends Frame 
implements ActionListener {
  ...
  //* 貪欲法によるMST構築
  //* このアルゴリズムでは採用した辺を配列に格納する必要がなく、
  //* edge[i]を採用するときには、edge[i].inMSTをtrueにするだけでよい。
  private void makeMSTByGreedy() {
	    // 点の対のすべてに対して辺を作る
	    makeEdges();

	    //* 生成した辺を重みの小さい順に整列する
	    Arrays.sort(edge,new WeightComaparator());
	   
	    //* すべてのedge[j].inMstをfalseに初期化 
	   ...
	   for(int i=0;i<ne;i++) {
		 ...
		 //* これを以下のif文の条件に用いることで閉路を検出する
			...
			//* edge[i]を採用しても閉路にならない場合
			//* (辺の両端点の属する連結成分の代表頂点が異なる、つまり
			//* 辺の両端点が異なる連結成分に属するときにはその木を採用しても
			//* 閉路にはならない)
			if(vv[0]!=vv[1]) {
				edge[i].inMST = true;
  ...
  private class WeightComaparator implements Comparator<Edge>{
	  //* 辺をその重みによって比較する
	  public int compare(Edge e,Edge f) {
		  if(e.weight<f.weight)
			  return -1;
		  else if(f.weight<e.weight)
			  return 1;
		  else
			  return 0;
	  }
