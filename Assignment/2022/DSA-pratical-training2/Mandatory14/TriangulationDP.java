/*
凸多角形の最小重み三角化の３回目です。
前回に引き続き厳密な最適解を求めるための動的計画法アルゴリズムを実装
します。作成した表をもとにして、最適な三角化そのものを求め画面に表示する
ようにしてください。

課題は以下の通りです。
1. 「DP」ボタンを押したときに動的計画法によって最適な分割を求め、画面
に表示するようにする。
最適な分割で使用する対角線は、貪欲法で使用している Diagonal 型の配列
diagonal に格納するようにします。その際、使用した対角線の本数 nd も忘れず
に設定するようにしてください。
*/
...
implements ActionListener {
  ...
  //* メソッドtriangulate内で対角線を採用するために使用するリスト
  ArrayList<Diagonal> diagonal_list;
...
//* 最適な分割で使用する対角線のリストアップに用いるメソッド
  private void triangulate(int i, int j) {
	  //* 再帰の基底
	  if(j == i+1) {
		  return;
	  }
	  
	  int k = bestDivider[i][j];
	  //* 頂点i,kを両端点とする線分が対角線であるときその対角線を採用する
	  if(k != i+1) {
		  diagonal_list.add(new Diagonal(i,k));
		  nd++;
	  }
	  //* 頂点k,jを両端点とする線分が対角線であるときその対角線を採用する
	  if(j != k+1) {
		  diagonal_list.add(new Diagonal(k,j));
		  nd++;
	  }
	  
	  //* 求めたい解の形になるように再帰呼び出しを行う
	  triangulate(i,j-1);
	  triangulate(i+1,np-1);
  }
  ...
  private void dp() {
	  ...
	  //* リストを初期化する
	  diagonal_list = new ArrayList<Diagonal>();
	
	  //* 使用した対角線の本数を覚える変数ndを初期化する
	  nd = 0;
      ...
      //* 採用した対角線を配列diagonalに格納する
      diagonal = new Diagonal[nd];
      for(int x=0;x<diagonal_list.size();) {
    	  //* 対角線どうしが交わらないようにする
    	  xx = diagonal_list.get(x);
    	  if(xx.crossesNone(diagonal,x))
    		  diagonal[x++] = xx;
      }
